...
class LeverEstimatorNode:
    ...
    def __call__(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        ranked_levers = sorted(
            lever_impacts, 
            key=lambda x: float(x.get("expected_impact", 0.0)), 
            reverse=True
        )
    ...
    def _estimate_lever_impacts(
        ...
        for hyp in validated_hypotheses:
            ...
            # Handle confidence as either float or Confidence enum
            conf_value = confidence if isinstance(confidence, (int, float)) else 0.5
            lever_map[lever_name]["confidence"] = max(
                lever_map[lever_name]["confidence"], conf_value
            )
