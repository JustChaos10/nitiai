...
from ..utils.hypothesis_utils import coerce_to_float, hypothesis_to_dict
...
class ExplanationGeneratorNode:
    ...
    def _generate_with_llm(
        ...
        for hyp in validated_hypotheses:
            ...
            p_value = coerce_to_float(consensus.get("p_value"), 1.0)
            effect_size = coerce_to_float(consensus.get("effect_size"), 0.0)
            confidence = coerce_to_float(consensus.get("confidence"), 0.0)
...
Explanation:"""
    ...
    def _generate_structured_explanation(
        ...
        if strongest:
            ...
            effect_size = coerce_to_float(
                strongest.get("consensus", {}).get("effect_size", 0.0), 0.0
            )
            parts.append(
