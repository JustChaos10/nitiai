...
# Suppress NumPy deprecation warnings for boolean indexing
...
class StatisticalTests:
    ...
    def simple_ttest(
        ...
        """Simple t-test comparing treatment vs control groups.
        ...
        try:
            ...
            if len(data) < 20:
                return TestResult(
                    ...
                    warnings=["Insufficient sample size (< 20)"],
                )

            # Split by treatment status
            treated = data[data["treatment"] == 1]["outcome"].values
            control = data[data["treatment"] == 0]["outcome"].values
            ...
            if len(treated) < 5 or len(control) < 5:
                return TestResult(
                    ...
                    is_significant=False,
                    confidence=Confidence.LOW,
                    warnings=["Too few samples in one group"],
                )
            ...
            return TestResult(
                ...
                effect_direction="positive" if mean_diff > 0 else "negative",
